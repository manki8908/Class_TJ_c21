...
class CrudTable():
    ...
    def select_all_record(self):
        ...
        try:
            ...
            print(f"===상품 전체조회(검색된 레코드수: {len(rows)})===")
    ...
    def select_one_record(self):
        ...
        try:
            ...
            while (True) :
                ...
                farg = input("조회 코드 or 상품명 입력 : (조회를 끝내고 싶으면 엔터)   ")
                ...
                if farg.isdecimal():    # 정수형으로 반환 가능하면 code로 인식
                    # 정수형으로 반환 가능
                    farg = int(farg)                    
                    sql = f"select * from goods where code = {farg}"
                else:
                    # 정수형으로 반환 불가능
                    farg = farg.strip()
                    sql = f"select * from goods where name like '%{farg}%'"
                ...
                if rows: 
                    ...
                    print("조회결과 입력한 이름에 맞는 상품이 없습니다. 다시 입력해주세요")
    ...
    def delete_record(self):
        ...
        try:
            ...
            while (True) :
                ...
                # 삭제 진행
                if self.is_it_there: 
                    print("삭제 상품이 존재합니다, 삭제를 진행합니다.")   


                    # -------------- 단일 삭제 상품 조회 및 삭제
                    # .. 삭제                          
                    if self.find_arg.isdecimal():    # 정수형으로 반환 가능하면 code로 인식
                        # 정수형으로 반환 가능
                        darg = int(self.find_arg)                        
                        sql = f"delete from goods where code = {darg}"
                    else:
                        # 정수형으로 반환 불가능
                        darg = self.find_arg.strip()
                        sql = f"delete from goods where name like '%{darg}%'"
